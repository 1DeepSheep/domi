...
- **待办事项**：把关键节点、新入库对象、重点项目／人物动态和长期未跟进事项维护到飞书 `1.待办事项` 或本地 `0.待办事项.md`，在客户端看板中执行或忽略。
...
1. 选择“本地资料库”或“飞书资料库”；
2. 使用待办事项看板时，飞书模式会自动发现或创建 `1.待办事项`，本地模式会在工作区根目录初始化 `0.待办事项.md`；需要约日程时可只读检测 Outlook 当前发送账号，并配置默认时区和多个常用参会人；
...
| 模式 | 适合谁 | 数据如何保存 |
| --- | --- | --- |
| 本地资料库 | 希望开箱即用、资料不依赖第三方云服务的用户 | SQLite 保存结构化索引和状态；`0.待办事项.md`、其他 Markdown、图片及附件统一放在“所选目录 / domi工作区”中 |
...
- 飞书、PLAUD、`1.待办事项` 文档、Outlook 上次验证的发送账号、常用参会人和目录映射由每位用户在自己的 Mac 上配置并保存在本地。
