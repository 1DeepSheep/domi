...
在右侧 PLAUD 列表点击“生成纪要并入库”后，对话中会立即显示会议背景卡片。卡片先展示已有文字稿的原文线索，后台补充简短的回忆提示；填写期间无需等待完整纪要工作流启动。确认会议类型、项目简称和已知参会者后即可生成，也可以选择“直接处理”。草稿和已确认信息会保留，重启或重试无需重复填写；回忆提示按录音账号及文字稿版本缓存。正式纪要仍读取完整文字稿并执行信息覆盖、证据、格式和归档核验。
