...
test("scans Codex and Agents roots, registers Codex skills in place, and keeps official skills isolated", () => {
  ...
  try {
    ...
    assert.equal(local.suggestedName, "research");
    ...
    const importedLocal = result.imported.find((skill) => skill.name === "research");
    const importedAgents = result.imported.find((skill) => skill.name === "notes");
    assert.equal(importedLocal.path, fs.realpathSync(localPath));
...
test("a damaged or newer registry is preserved instead of silently becoming an empty catalog", () => {
  ...
  try {
    ...
    assert.equal(fixture.service.listImported().skills[0].name, "keep-me");
...
test("refreshes edited metadata and multiline YAML without changing workflow identity", () => {
  const fixture = createFixture();
  try {
    const skillPath = writeSkill(path.join(fixture.codexHome, "skills"), "editable");
    ...
    fs.writeFileSync(path.join(skillPath, "SKILL.md"), [
      "---", "name: editable", 'title: "Edited: Skill"', "description: >",
...
test("safe renames retain queued workflow identity and old aliases cannot capture it", () => {
  ...
  try {
    const codexSkills = path.join(fixture.codexHome, "skills");
    const skillPath = writeSkill(codexSkills, "original", { name: "original" });
    const initial = fixture.service.import({ candidateIds: fixture.service.scan().candidates.map((item) => item.id) });
    const queuedWorkflowId = initial.skills[0].id;
    writeSkill(codexSkills, "original", { name: "renamed" });
    ...
    writeSkill(codexSkills, "unrelated", { name: "original" });
    const replacement = fixture.service.scan().candidates.find((item) => item.name === "original");
...
test("conflicting and invalid edits are unavailable, visible, and recover with the same id", () => {
  ...
  try {
    const codexSkills = path.join(fixture.codexHome, "skills");
    const skillPath = writeSkill(codexSkills, "first");
    ...
    writeSkill(codexSkills, "first", { name: "second" });
    ...
    assert.equal(conflicted.candidates.find((item) => item.sourcePath === fs.realpathSync(skillPath)).status, "unavailable");
    const second = conflicted.candidates.find((item) => item.sourcePath !== fs.realpathSync(skillPath));
    ...
    writeSkill(codexSkills, "first", { name: "repaired" });
...
test("legacy registry ids survive metadata refresh and failed imports report catalog changes", () => {
  ...
  try {
    ...
    writeSkill(codexSkills, "legacy", { name: "renamed-legacy" });
    const result = fixture.service.import({ candidateIds: [candidate.id] });
    assert.equal(result.ok, false);
    assert.equal(result.imported.length, 0);
...
test("concurrent newly created Skills require explicit selection instead of automatic import", async () => {
  ...
  try {
    ...
    assert.equal(confirmed.skills[0].name, "created-in-current-task");
...
test("Skill Hub UI stays compact and launches the native skill-creator conversation", () => {
  ...
  assert.match(manager, /从本机 Codex 导入[\s\S]*?selected[\s\S]*?importSelected/);
  assert.match(manager, /原生 skill-creator 边聊边创建/);
  assert.match(styles, /\.sidebar-workflow-section \.sidebar-workflows[\s\S]*?max-height:\s*min\(220px, 28vh\)[\s\S]*?overflow-y:\s*auto/);
...
test("Skill Hub disclosure and manage button have separate horizontal hit areas", () => {
  const styles = fs.readFileSync(path.resolve(__dirname, "../src/styles.css"), "utf8");
  const toggle = styles.match(/\.sidebar-skill-hub-header \.sidebar-section-toggle\s*\{([^}]+)\}/)[1];
  const manage = styles.match(/\.sidebar-skill-hub-manage\s*\{([^}]+)\}/)[1];
  const disclosure = styles.match(/\.sidebar-nav-disclosure\s*\{([^}]+)\}/)[1];
  const px = (block, property) => Number(block.match(new RegExp(`(?:^|[;\\s])${property}:\\s*(\\d+)px`))[1]);
  const headerWidth = 240;
  const arrowRight = headerWidth - px(toggle, "padding-right");
  const arrowLeft = arrowRight - px(disclosure, "width");
  const manageRight = headerWidth - px(manage, "right");
  const manageLeft = manageRight - px(manage, "width");
  assert.ok(manageRight <= arrowLeft || arrowRight <= manageLeft,
    `Skill Hub controls overlap: arrow=[${arrowLeft},${arrowRight}], manage=[${manageLeft},${manageRight}]`);
  assert.ok(arrowLeft - manageRight >= 4, "keep a visible gap with the manage button to the left of disclosure");
});
