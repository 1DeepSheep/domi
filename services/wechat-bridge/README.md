...
- 每个新任务立即返回 `W01`–`W20` 范围内的短编号。
- 不同任务使用不同 Codex thread；引用 `W01`、`1号任务`或回复带编号的消息可继续原任务。
- 默认显示最近 5 项、保留最近 10 项已结束任务；运行中、等待补充或结果待发送的任务不会被清理。
- 最终结果会先保存到本机待发送队列。微信临时拒绝发送时，下一条消息会带来新会话上下文并自动补发，不会把已完成任务误标成执行失败。
- 长任务只在约 3 分钟和 10 分钟发送里程碑状态，避免重复进度耗尽最终结果与附件的回复额度。
...
- `/help`：查看帮助。
- `/status [W01]`：查看桥接与任务状态。
- `/tasks`：查看最近任务。
- `/cancel W01`：取消运行或排队任务。
